...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!hasAccess) return; // Block execution if unauthorized
    ...
    async function fetchData() {
        try {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Loading secure data...</td></tr>';

            const { data: srvData, error: srvError } = await supabase.from('services').select('*');
            if (srvError) throw srvError;
            if (srvData) allServices = srvData;

            const { data: docData, error: docError } = await supabase.from('doctors').select('*');
            ...
            if (docData) {
                ...
                if(walkinDocSelect && doctorsList.length > 0) {
                    walkinDocSelect.innerHTML = '<option value="" disabled selected>Please select a doctor</option>' +
    ...
    function updateDoctorDropdown() {
        ...
        doctorsList.forEach(doc => {
            ...
            option.textContent = pendingCount > 0 ? `${docName} (${pendingCount} New)` : docName;
    ...
    function updateStats() {
        ...
        allAppointments.forEach(app => {
            ...
            if (app.appointmentDate === todayStr && app.status !== 'کنسل شده' && app.status !== 'رد شده') {
                todayCount++;
            }
        });
    ...
    function renderTable(filteredData) {
        ...
        if (filteredData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center" style="padding: 40px 0; color: #64748b;">No appointments found.</td></tr>';
            return;
        }
        ...
        const groupedData = filteredData.reduce((acc, app) => {
            const doctor = app.doctorName || 'Unknown Doctor';
            if (!acc[doctor]) {
                acc[doctor] = [];
            }
            acc[doctor].push(app);
        ...
        for (const [docName, apps] of Object.entries(groupedData)) {
            ...
            trTitle.innerHTML = `
                <td colspan="5" style="background-color: #f1f5f9; font-family: 'Title', sans-serif; font-size: 16px; color: #0f172a; text-align: right; padding: 12px 20px; border-radius: 8px;">
                    <i class="fas fa-user-md" style="color:var(--color-primary); margin-left:8px;"></i>
                    ${docName}
                    <span style="font-size: 12px; color: #64748b; margin-right: 10px; font-family: 'TextRegular', sans-serif;">(${toPersianNumber(apps.length)} items)</span>
                </td>
            `;
            ...
            apps.forEach(app => {
                ...
                const patientName = app.patient_name || 'N/A';
                ...
                const notes = app.patient_notes || app.notes || '';
                ...
                tr.innerHTML = `
                    <td>
                        ...
                        </div>` : '<small style="color:#94a3b8;">No Number</small>'}
                    </td>
                    <td>
                        <div style="font-weight:600; color:#334155;">${app.serviceName || 'General Visit'}</div>
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <span class="payment-badge ${isPaid ? 'paid' : ''}">${isPaid ? '<i class="fas fa-check-circle"></i> Paid' : 'Unpaid'}</span>
                    </td>
                    <td>
                        <div class="s-actions-group">
                            ${app.status === 'در حال بررسی' ? `
                                <button class="s-btn-action approve js-action-approve" data-id="${app.id}"><i class="fas fa-check"></i></button>
                                <button class="s-btn-action reject js-action-reject" data-id="${app.id}"><i class="fas fa-times"></i></button>
                            ` : ''}
                            ${app.status === 'تایید شده' ? `
                                ${!isPaid ? `<button class="s-btn-action pay js-action-pay" data-id="${app.id}"><i class="fas fa-dollar-sign"></i></button>` : ''}
                                <button class="s-btn-action delete js-action-delete" data-id="${app.id}"><i class="fas fa-trash"></i></button>
                            ` : ''}
                        </div>
    ...
    async function handleStatusChange(id, newStatus) {
        ...
        if (newStatus === 'تایید شده') {
            ...
            if (targetApp) {
                ...
                if (hasConflict) {
                    await window.customModal.alert('Conflict', 'Time slot already approved for this doctor.');
                    return;
                }
            }
        }
        const res = await window.customModal.confirm('Status Update', `Are you sure you want to change status to "${newStatus}"?`);
        if (res === 'confirm') {
            await window.appointmentsService.updateAppointmentStatus(id, newStatus, '');
            if (newStatus === 'تایید شده') {
                await window.customModal.alert('System Info', 'Appointment approved successfully.');
            }
            await fetchData();
        }
    }
    ...
    async function handleDelete(id) {
        if (!window.customModal) return;
        const res = await window.customModal.confirm('Delete', 'Are you sure you want to delete this appointment?');
        if (res === 'confirm') {
    ...
    async function handlePayment(id) {
        if (!window.customModal) return;
        const res = await window.customModal.confirm('Payment', 'Mark as paid?');
        if (res === 'confirm') {
    ...
    function showPatientHistory(phone, name) {
        if (!phone) {
            window.customModal.alert('Error', 'Patient phone not found.');
            return;
        }
        const history = allAppointments.filter(a => a.patient_phone === phone);
        let html = `<h4 style="margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">History: ${name}</h4>`;
        ...
        if(history.length === 0) {
            ...
            history.forEach(h => {
                ...
                <div style="background:#f8fafc; padding:10px; border-radius:8px; margin-bottom:10px; border:1px solid #e2e8f0;">
                    ...
                    <small>Doc: ${h.doctorName} | Status: ${h.status}</small>
                </div>`;
            });
        }
        ...
        if (matches.length > 0) {
            ...
            userSearchResults.innerHTML = matches.map(u => `
                <div class="s-search-item" data-id="${u.id}" data-name="${u.name || u.userName} ${u.family || u.userFamily || ''}" data-phone="${u.phone || u.userPhone || ''}">
                    <strong>${u.name || u.userName} ${u.family || u.userFamily || ''}</strong> - ${u.phone || u.userPhone || 'N/A'}
                </div>
            `).join('');
            ...
            userSearchResults.querySelectorAll('.s-search-item').forEach(item => {
                item.addEventListener('click', (ev) => {
                    ...
                    selectedUserDisplay.textContent = `Selected: ${walkinState.patientName} (${toPersianNumber(walkinState.patientPhone)})`;
                    userSearchResults.style.display = 'none';
            ...
            userSearchResults.style.display = 'block';
            userSearchResults.innerHTML = '<div style="padding:10px; color:#94a3b8; font-size:12px;">No patient found.</div>';
        }
    });
    ...
    walkinDoctorSelect.addEventListener('change', (e) => {
        ...
        wTimeSlots.innerHTML = '<p style="font-size:12px; color:#64748b;">Select date first</p>';
        ...
        const docServices = allServices.filter(s =>
            ...
            String(s.doctor_id) === String(docId) ||
            s.doctorName === docName ||
            `دکتر ${s.doctorName}` === docName ||
            s.doctor === docName ||
            `دکتر ${s.doctor}` === docName
        );

        walkinServiceSelect.innerHTML = '<option value="" disabled selected>Please select a service</option>';
        ...
        if (docServices.length > 0) {
            ...
            const opt = document.createElement('option');
            opt.value = 'General Visit';
            opt.textContent = 'General Visit';
            walkinServiceSelect.appendChild(opt);
        }
    ...
    document.getElementById('js-submit-walkin').addEventListener('click', async () => {
        ...
        if (isExisting) {
            if (!walkinState.userId) { return window.customModal.alert('Error', 'Please select a patient.'); }
            ...
            const nAge = document.getElementById('new-user-age').value.trim();

            if(!nName || !nPhone) { return window.customModal.alert('Error', 'Name and Phone required.'); }
            ...
            try {
                ...
                if (authErr && authErr.message.includes('already registered')) {
                    return window.customModal.alert('Error', 'Phone already registered.');
                }
                ...
                const newUserPayload = {
                    ...
                    family: nFamily,
                    phone: nPhone,
                    age: nAge ? String(nAge) : null,
                    email: fakeEmail,
                    role: 'user',
                    created_at: new Date().toISOString()
                };

                const { error: profileErr } = await supabase.from('profiles').upsert([newUserPayload]);
                ...
                return window.customModal.alert('Error', e.message);
            }
        }

        if (!walkinServiceSelect.value) {
            return window.customModal.alert('Error', 'Please select a service.');
        }

        if (!walkinState.date || !walkinState.time) {
            return window.customModal.alert('Error', 'Please select date and time.');
        }

        const notesInput = document.getElementById('walkin-notes').value.trim();
        const selectedDocOpt = walkinDoctorSelect.options[walkinDoctorSelect.selectedIndex];
        const finalDoctorName = selectedDocOpt ? selectedDocOpt.dataset.name : walkinDoctorSelect.value;
        ...
        try {
            ...
            window.customModal.alert('Success', 'Appointment booked.');
            walkinModal.close();
            ...
            walkinServiceSelect.value = '';
            selectedUserDisplay.style.display = 'none';
            walkinServiceContainer.style.display = 'none';
            walkinCalendarWrapper.style.display = 'none';
            walkinState = { userId: null, patientName: '', patientPhone: '', date: null, time: null, notes: '' };

            await fetchData();
        } catch(err) {
            window.customModal.alert('Error', err.message);
        }
    });
