...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        for (const [docName, apps] of Object.entries(groupedData)) {
            ...
            trTitle.innerHTML = `
                <td colspan="5" style="background-color: #f1f5f9; font-family: 'Title', sans-serif; font-size: 16px; color: #0f172a; text-align: right; padding: 12px 20px; border-radius: 8px;">
                    ...
                    ${docName}
                    <span style="font-size: 12px; color: #64748b; margin-right: 10px; font-family: 'TextRegular', sans-serif;">(${toPersianNumber(apps.length)} نوبت)</span>
                </td>
            `;
    ...
    async function handleStatusChange(id, newStatus) {
        ...
        if (res === 'confirm') {
            ...
            if (newStatus === 'تایید شده') {
                await window.customModal.alert('اطلاع‌رسانی سیستم', 'نوبت با موفقیت تایید شد و پیامک تایید برای بیمار ارسال گردید.');
            }
        ...
        if (matches.length > 0) {
            ...
            userSearchResults.querySelectorAll('.s-search-item').forEach(item => {
                item.addEventListener('click', (ev) => {
                    ...
                    selectedUserDisplay.textContent = `بیمار انتخاب شد: ${walkinState.patientName} (${toPersianNumber(walkinState.patientPhone)})`;
                    userSearchResults.style.display = 'none';
        ...
        if (isExisting) {
            ...
            try {
                ...
                if (authErr && authErr.message.includes('already registered')) {
                    return window.customModal.alert('خطا', 'این شماره قبلا در سیستم ثبت شده است.');
                }
                ...
                if (profileErr) {
                    ...
                    throw new Error('ذخیره در جدول پروفایل با مشکل مواجه شد: ' + profileErr.message);
                }
                ...
                return window.customModal.alert('خطا', 'ساخت حساب کاربری با مشکل مواجه شد: ' + e.message);
            }
        }

        if (!walkinServiceSelect.value) {
            return window.customModal.alert('خطا', 'لطفا خدمت مورد نظر را انتخاب کنید.');
        }

        if (!walkinState.date || !walkinState.time) {
            return window.customModal.alert('خطا', 'تاریخ و ساعت را انتخاب کنید.');
        }
