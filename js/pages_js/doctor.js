...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadAppointments() {
        ...
        try {
            ...
            // Smart Filter: find appointments for this specific doctor
    ...
    function timeToMinutes(timeStr) {
        ...
        const startHourStr = englishTime.split(':')[0];
    ...
    function renderTable(filter) {
        ...
        for (const [dateKey, apps] of Object.entries(groupedByDate)) {
            ...
            // Sort by Time
            apps.sort((a, b) => {
                const timeA = timeToMinutes(a.appointmentTime || a.time);
                const timeB = timeToMinutes(b.appointmentTime || b.time);
                return timeA - timeB;
            });
            ...
            dateHeaderRow.innerHTML = `
                <td colspan="5" style="background: #f1f5f9; color: #0077c0; font-family: 'Title', sans-serif; font-size: 16px; padding: 12px 20px; text-align: right; border-bottom: 2px solid #e2e8f0;">
                    <i class="far fa-calendar-alt" style="margin-left: 8px;"></i> نوبت‌های تاریخ: ${dateKey}
                </td>
            `;
            ...
            apps.forEach(app => {
                ...
                tr.innerHTML = `
                    <td>
                        ...
                        ${app.patient_phone ? `<div style="font-size:12px; color:#475569; margin-bottom:6px;"><i class="fas fa-phone-alt" style="font-size:10px; margin-left:4px;"></i>${app.patient_phone}</div>` : ''}
                        ${paymentBadge}
                    </td>
                    <td>
                    ...
                    <td>
                        <div dir="ltr" style="font-size: 14px; color:#334155; font-weight:bold;">
                            <i class="far fa-clock" style="color:#94a3b8; margin-right:4px;"></i> ${app.appointmentTime || app.time}
                        </div>
                    </td>
                    <td><span class="status-badge ${statusClass}">${app.status}</span></td>
                    <td>
                        <div class="d-actions-group">
                            <button class="d-btn-action js-action-history" data-patient-id="${app.userId || ''}" data-patient-name="${patientName}" style="background:#64748b;">
    ...
    function attachActionEvents() {
        ...
        // Status Update (3-options) + Auto Note Modal
            ...
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    if (res === 'confirm') {
                        ...
                        // Auto-open note modal
                        currentEditingAppointmentId = id;
                        notePatientName.textContent = name;
                        noteTextarea.value = '';
        ...
        document.querySelectorAll('.js-action-history').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!patientId || patientId === 'null') {
                    historyContent.innerHTML = `<div style="padding: 20px; color: #ef4444; text-align:center;">این بیمار به صورت مهمان (بدون اکانت) توسط منشی ثبت شده و پرونده سیستمی ندارد.</div>`;
                ...
                try {
                    ...
                    if (!patientHistory || patientHistory.length === 0) {
                        historyContent.innerHTML = `<div style="padding: 20px; text-align:center;">سابقه‌ای برای این بیمار یافت نشد.</div>`;
                        return;
                    }

                    const historyHtml = patientHistory.map(hist => {
                        let borderColor = '#cbd5e1';
                        if(hist.status === 'ویزیت شده') borderColor = '#10b981';
                        else if(hist.status === 'عدم مراجعه') borderColor = '#64748b';
                        else if(hist.status === 'رد شده' || hist.status === 'کنسل شده') borderColor = '#ef4444';
                    ...
                    historyContent.innerHTML = `<div style="padding: 20px; color: #ef4444; text-align:center;">خطا در دریافت پرونده: ${err.message}</div>`;
                }
            });
        });
    }
    ...
    if (saveNoteBtn) {
        saveNoteBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (window.customModal) await window.customModal.alert('خطا', 'خطا در ذخیره نسخه: ' + err.message);
            } finally {
                saveNoteBtn.disabled = false;
                saveNoteBtn.innerHTML = 'ذخیره در پرونده';
            }
        });
    }
    ...
    function setupRealtime() {
        supabase.channel('public:doctor-appointments')
